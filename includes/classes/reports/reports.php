...
class reports
{
    ...
    public static function add_filters_query($reports_id, $listing_sql_query='', $prefix = '', $is_parent_report = false,$exclude_fields = [])
    {
        ...
        if ($reports_info = db_fetch_array($reports_info_query))
        {
            ...
            elseif ($app_entities_cache[$reports_info['entities_id']]['parent_id'] > 0 and in_array($reports_info['reports_type'], ['default','common'])) //check access for report type 'default' where parent_id=0
            {
    ...
    {
        $reports_query = db_query("select * from app_reports where entities_id='" . db_input($entities_id). "' and reports_type='{$type}'");
        if($reports = db_fetch_array($reports_query))
        {
            return $reports['id']; 
        }
