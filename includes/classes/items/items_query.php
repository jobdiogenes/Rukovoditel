...
class items_query
{
    ...
    function __construct($entity_id,$settings)
    {
        ...
        {
            $report_query = db_query("select r.id, r.listing_order_fields, (select group_concat(rf.fields_id) from app_reports_filters rf where rf.reports_id=r.id) as fields_in_query from app_reports r where r.id='" . $this->settings->get('report_id') . "'");
