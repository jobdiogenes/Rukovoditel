...
class items
{
    ...
    public static function render_content_box($entity_id, $item_id, $users_id = false)
    {
        ...
        $html = '';
        $fields_query = db_query("select f.*, fr.sort_order as form_rows_sort_order,right(f.forms_rows_position,1) as forms_rows_pos from app_fields f left join app_forms_rows fr on fr.id=LEFT(f.forms_rows_position,length(f.forms_rows_position)-2), app_forms_tabs t where f.type in ('fieldtype_image_map_nested','fieldtype_textarea_encrypted','fieldtype_video','fieldtype_iframe','fieldtype_google_map_directions','fieldtype_google_map','fieldtype_yandex_map','fieldtype_yandex_map_directions','fieldtype_mind_map','fieldtype_image_map','fieldtype_todo_list','fieldtype_textarea','fieldtype_textarea_wysiwyg','fieldtype_attachments','fieldtype_image','fieldtype_image_ajax','fieldtype_mapbbcode') and  f.entities_id='" . db_input($entity_id) . "' and f.forms_tabs_id=t.id order by t.sort_order, t.name, form_rows_sort_order, forms_rows_pos,  f.sort_order, f.name");
        while ($field = db_fetch_array($fields_query))
        {
            //exclude fields in email
            if ($is_email and in_array($field['type'], fields_types::get_types_excluded_in_email()))
